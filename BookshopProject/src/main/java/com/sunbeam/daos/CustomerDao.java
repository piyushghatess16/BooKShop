...
public class CustomerDao implements Closeable {
	...
	public Customer findById(int custId) throws Exception {
		String sql = "SELECT c.id, c.name, c.password, c.mobile, c.address, c.email, c.birth, c.enabled, r.role FROM customers c INNER JOIN roles r ON c.email=r.email WHERE c.id=?";
	...
	public Customer findByEmail(String email) throws Exception {
		String sql = "SELECT c.id, c.name, c.password, c.mobile, c.address, c.email, c.birth, c.enabled, r.role FROM customers c INNER JOIN roles r ON c.email=r.email WHERE c.email=?";
		try(PreparedStatement stmt = con.prepareStatement(sql)) {
